...
from .models.responses import ServersResponse, ErrorResponse, PiImagesResponse
...
class PiCloud:
    ...
    def pis(self) -> dict[str, Pi]:
        ...
        data = ServersResponse.model_validate(response.json())

        return {
            name: Pi(name=name, api_url=self._api_url, session=self.session)
    ...
    def create_pi(
        ...
        *,
        ...
        response = self.session.post(url, json=data.spec.model_dump())
        ...
        try:
            ...
            if response.status_code == 400:
                ...
                raise HostedPiException(f"Invalid parameters: {error}") from exc
            if response.status_code == 403:
        ...
        return Pi(name=name, api_url=self._api_url, session=self.session)
